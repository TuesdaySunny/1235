function [path, path_found] = RRT(robot, q_min, q_max, q_start, q_goal, link_radius, ...
    ...
    % 参数设置
    max_iterations = 100000; % RRT搜索的最大迭代次数
    step_size = 2;           % 基础步长，用于扩展节点
    ...
    threshold = 100; % 当节点到目标的距离小于该阈值时，认为达到了目标
...
%             % 在目标邻域采样
%             distanceToGoal = vecnorm(t.v(end).q - q_goal, 2);
%             if distanceToGoal < goalProximityThreshold
%                 q_rand = q_goal;
%             else
%                 theta = rand * 2 * pi;
%                 phi = acos(2 * rand - 1);
%                 radius = goalSampleRadius * (rand^(1/3));
%                 q_rand = q_goal + radius * [cos(theta) * sin(phi), sin(theta) * sin(phi), cos(phi)];
%             end
%         end
 ...
 % 沿着最近节点向随机配置扩展
        delta_q = step_size * (q_rand - q_near) / min_dist; % 计算增量
        q_new = q_near + delta_q; % 新配置
